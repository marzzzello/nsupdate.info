...
class BlacklistedHost(models.Model):
    name_re = models.CharField(
        _('name RegEx'), max_length=255, unique=True, help_text=_('Blacklisted domain. Evaluated as regex (search).')
    )
...
UPDATE_ALGORITHMS = {
    ...
    'HMAC_SHA512': UpdateAlgorithm(
        512,
        'hmac-sha512',
    ),
    'HMAC_SHA384': UpdateAlgorithm(
        384,
        'hmac-sha384',
    ),
    'HMAC_SHA256': UpdateAlgorithm(
        256,
        'hmac-sha256',
    ),
    'HMAC_SHA224': UpdateAlgorithm(
        224,
        'hmac-sha224',
    ),
    'HMAC_SHA1': UpdateAlgorithm(
        160,
        'hmac-sha1',
    ),
    'HMAC_MD5': UpdateAlgorithm(
        128,
        'hmac-md5',
    ),
}
...
class Domain(models.Model):
    ...
    nameserver_update_algorithm = models.CharField(
        ...
        default=UPDATE_ALGORITHM_DEFAULT,
        choices=UPDATE_ALGORITHM_CHOICES,
    ...
    public = models.BooleanField(
        ...
        default=False,
        help_text=_(
            "Check to allow any user to add dynamic hosts to this zone - "
    ...
    available = models.BooleanField(
        ...
        default=False,
        help_text=_(
            "Check if nameserver is available/reachable - "
    ...
    comment = models.CharField(
        ...
        null=True,
        help_text=_(
            "Some arbitrary comment about your domain. "
    ...
    created = models.DateTimeField(_("created at"), auto_now_add=True)
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name='domains', verbose_name=_("created by"), on_delete=models.CASCADE
    )
...
class Host(models.Model):
    ...
    available = models.BooleanField(
        _("available"),
        default=True,
        help_text=_("Check if host is available/in use - " "if not checked, we won't accept updates for this host"),
    )
    netmask_ipv4 = models.PositiveSmallIntegerField(
        _("netmask IPv4"), default=32, help_text=_("Netmask/Prefix length for IPv4.")
    )
    netmask_ipv6 = models.PositiveSmallIntegerField(
        _("netmask IPv6"), default=64, help_text=_("Netmask/Prefix length for IPv6.")
    )
    ...
    abuse = models.BooleanField(
        ...
        default=False,
        help_text=_(
            "Checked if we think you abuse the service - " "you may uncheck this AFTER fixing all issues on your side"
        ),
    )

    # similar to above, but can not be toggled by the user:
    abuse_blocked = models.BooleanField(
        _("abuse blocked"), default=False, help_text=_("Checked to block a host for abuse.")
    )
    ...
    created = models.DateTimeField(_("created at"), auto_now_add=True)
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name='hosts', verbose_name=_("created by"), on_delete=models.CASCADE
    )
    ...
    def generate_secret(self, secret=None):
        ...
        self.update_secret = make_password(secret, hasher='sha1')
        self.save()
        return secret
...
class RelatedHost(models.Model):
    ...
    main_host = models.ForeignKey(
        Host, on_delete=models.CASCADE, related_name='relatedhosts', verbose_name=_("main host")
    )
...
class ServiceUpdater(models.Model):
    name = models.CharField(_("name"), max_length=32, help_text=_("Service name"))
    comment = models.CharField(
        _("comment"),
        max_length=255,  # should be enough
        default='',
    ...
    server = models.CharField(
        _("server"), max_length=255, help_text=_("Update Server [name or IP] of this service")  # should be enough
    )
    ...
    secure = models.BooleanField(
        _("secure"), default=True, help_text=_("Use https / TLS to contact the Update Server?")
    )
    ...
    created = models.DateTimeField(_("created at"), auto_now_add=True)
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name='serviceupdater', verbose_name=_("created by"), on_delete=models.CASCADE
    )
...
class ServiceUpdaterHostConfig(models.Model):
    ...
    host = models.ForeignKey(
        Host, on_delete=models.CASCADE, related_name='serviceupdaterhostconfigs', verbose_name=_("host")
    )
